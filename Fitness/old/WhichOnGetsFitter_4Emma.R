...
df_all_Simple <- data.frame(labelUniq=unique(df_all$label), ID = NA,
...
for (i in 1:dim(df_all_Simple)[1]) {
  ...
  df_all_Simple$ID[i] <- paste(df_all$lab[sampleIdx[1]],df_all$treat[sampleIdx[1]],sep = "_")
...
ggplot(df_all_Simple, aes(ID, sampleMean, fill=factor(week))) +
  geom_boxplot()
...
df_Pairs_w0_1to15 <- df_Pairs_w0_1to15[complete.cases(df_Pairs_w0_1to15[ , 6]), ]
