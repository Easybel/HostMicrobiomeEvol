...
myBamPath="/zfs/omics/projects/cevolution/Out/bam"
...
ID=$(ls -1 $myDataTrim | grep 'Gent\|UVA' | grep "1P" | sed -n ''$i'p'| cut -d'_' -f1,2,3,4)
IDout=$ID"_2CE"
...
#  -1 $myDataTrim/$ID"_CE_removed_R1.fastq.gz" \
#  -2 $myDataTrim/$ID"_CE_removed_R2.fastq.gz" \
#  -0 /dev/null -s $myDataTrim/$ID"_CE_removed_S.fastq.gz" -n
...
samtools fastq -f 1 -F 4 -F 256 -@ 8 $myBamPath/$IDout"_all_sort.bam" \
  -1 $myDataTrim/new/$ID"_CE_R1.fastq.gz" \
  -2 $myDataTrim/new/$ID"_CE_R2.fastq.gz" \
  -0 /dev/null -s $myDataTrim/new/$ID"_CE_S.fastq.gz" -n
