...
ID=$(ls -1 $myDataTrim | grep 'WUR\|RUG' | grep "1P" | sed -n ''$i'p'| cut -d'_' -f1,2,3,4)
IDout=$ID"_2CE"
...
samtools fastq -f 4 -F 256 -@ 8 $myBamPath/$IDout"_all_sort.bam" \
  -1 $myDataTrim/$ID"_woCE_R1.fastq.gz" \
  -2 $myDataTrim/$ID"_woCE_R2.fastq.gz" \
  -0 /dev/null -s $myDataTrim/$ID"_woCE_S.fastq.gz" -n
