...
SizeSubSamples=35	# how many reads to subsample per position
NumSubSamples=10 	# how often do we want to make a subsample
...
perl $popoPath1/basic-pipeline/subsample-pileup.pl --input $pilePath/$SamName"_temp.pileup" --output /scratch/easy/$SamNameOut".pileup" --target-coverage $SizeSubSamples --max-coverage 500 --min-qual 20 --method withoutreplace --fastq-type sanger


perl $popoPath1/Variance-sliding.pl --input /scratch/easy/$SamNameOut".pileup" --output $outPath/$SamNameOut"_Sanger_maxCov500.pi" --fastq-type sanger --measure pi --window-size 10000 --step-size 10000 --min-count 2 --min-coverage 4 --max-coverage 500 --pool-size 500 --min-covered-fraction 0.01

rm /scratch/easy/$SamNameOut".pileup"
done
