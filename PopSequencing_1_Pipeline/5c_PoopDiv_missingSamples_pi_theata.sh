...
# paths!
pilePath="/zfs/omics/projects/ecolievo/irathma/CnemEvol_EPB/PiDiversity/pileup_temp"
...
SamName=$(ls -1 $pilePath | grep "pileup" |grep "Gent_bb_15_5\|RUG_bb_15_3" | sed -n ''$i'p' | cut -d'_' -f1,2,3,4)

#perl $popoPath1/Variance-sliding.pl --input $pilePath/$SamName"_temp.pileup" --output $outPath/$SamName"_Sanger_maxCov500_fracCov_0.01.pi" --fastq-type sanger --measure pi --window-size 10000 --step-size 10000 --min-count 2 --min-coverage 4 --max-coverage 500 --pool-size 500 --min-covered-fraction 0.01
